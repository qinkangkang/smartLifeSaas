...
<button class="btn btn-white btn-sm" onclick="marketreturnAll()" data-toggle="tooltip" data-placement="top"><i class="fa fa-trash-o"> ${label==null?'退货':label}</i>
                        </button>
...
function marketreturnAll(){
	...
	if(selectedIps.length>0){
		...
		top.layer.confirm('确认要退货吗?', {icon: 3, title:'系统提示'}, function(index){
			window.location = "${ctx}/market/ordermarket/orderMarketDetail/marketreturnAll?ids="+ids;
