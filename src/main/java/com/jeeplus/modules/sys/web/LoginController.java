...
public class LoginController extends BaseController {
	...
	public String index(HttpServletRequest request, HttpServletResponse response) {
		...
		// try {
		// byte[] bytes =
		// com.jeeplus.common.utils.FileUtils.readFileToByteArray(
	...
	public String home(HttpServletRequest request, HttpServletResponse response, Model model) throws IOException {
		int count=warehouseTransferOrderService.findCountByType();//查找调拨中的挑拨单个数;
		int marketCount=orderMarketService.findCountByStatus();//待发货订单个数
		model.addAttribute("count",count);
		model.addAttribute("marketCount",marketCount);
		//查找最新的3条公告
		List<OaNotify> gonggaoList=oaNotifyService.findThreeNewOa();
		request.setAttribute("gonggaoList", gonggaoList);
		
		
		//查找未处理退货单的个数
		Integer countChangeBack = orderProChargebackService.countChangeBack(1);
		request.setAttribute("countChangeBack", countChangeBack);
